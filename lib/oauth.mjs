...
// 把 OAuth 错误 body 转成可读字符串（避免前端看到 "[object Object]"）
// Anthropic 有时返回 {error: "..."}（字符串）有时返回 {error: {type, message}}（对象）
function formatOAuthError(data, fallback, httpStatus) {
  if (!data || typeof data !== "object") {
    return `${fallback}${httpStatus ? ` (HTTP ${httpStatus})` : ""}`;
  }
  if (typeof data.error_description === "string") return data.error_description;
  if (typeof data.error === "string") return data.error;
  if (data.error && typeof data.error === "object") {
    // 常见形如 {type: "invalid_grant", message: "..."}
    const parts = [];
    if (data.error.type) parts.push(data.error.type);
    if (data.error.message) parts.push(data.error.message);
    if (parts.length) return parts.join(": ");
    try { return JSON.stringify(data.error); } catch { /* fallthrough */ }
  }
  return `${fallback}${httpStatus ? ` (HTTP ${httpStatus})` : ""}`;
}
...
export async function exchangeCode(code, state) {
  ...
  if (!res.ok) {
    throw new Error(formatOAuthError(data, "Token exchange failed", res.status));
  }
...
export async function refreshToken() {
  ...
  refreshPromise = (async () => {
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(formatOAuthError(data, "Token refresh failed", res.status));
      }
  ...
  refreshInterval = setInterval(async () => {
    ...
    if (status.expiresAt && status.expiresAt - Date.now() < margin) {
      try {
        ...
        console.error("[oauth] background refresh failed:", err?.message || String(err));
      }
    }
  }, 5 * 60 * 1000);
