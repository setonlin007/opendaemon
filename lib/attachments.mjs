...
/**
 * Update the metadata JSON for an attachment (used to store image generation params).
 * Pass an object; will be serialized to JSON string.
 */
export function updateAttachmentMetadata(attId, metadataObj) {
  const j = typeof metadataObj === "string" ? metadataObj : JSON.stringify(metadataObj ?? null);
  getDb().prepare("UPDATE attachments SET metadata = ? WHERE id = ?").run(j, attId);
}
