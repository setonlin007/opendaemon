...
function executeEditFile({ path: filePath, old_string, new_string }) {
...
function executeSearchFiles({ pattern, path: searchPath }) {
  try {
    const baseDir = searchPath ? resolveSafePath(searchPath, "read") : PROJECT_ROOT;
...
function executeSearchContent({ pattern, path: searchPath, glob: globFilter, max_results = 50 }) {
  try {
    const baseDir = searchPath ? resolveSafePath(searchPath, "read") : PROJECT_ROOT;
...
function executeRunShell({ command }) {
