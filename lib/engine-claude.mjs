...
export async function streamClaude({
  ...
  // API Key & platform via environment variables
  const apiKey = engineConfig.provider?.apiKey;
  const platform = engineConfig.provider?.platform;
  if (apiKey || platform) {
    options.env = { ...process.env };
    if (apiKey) options.env.ANTHROPIC_API_KEY = apiKey;
