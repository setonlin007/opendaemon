...
// Lazy-init Anthropic client for multimodal
...
export async function streamClaude({
  ...
  // If prompt is an array (content blocks with images/files), use raw Messages API
  // The Agent SDK only supports string prompts
  if (Array.isArray(prompt)) {
    return await runMultimodalQuery(prompt, injectedContext, convId, onEvent, abortSignal);
  }
  ...
  const options = {
    ...
    systemPrompt: buildSystemPrompt(convId, injectedContext),
  };
...
function buildSystemPrompt(convId, injectedContext) {
  return [
...
/**
 * Handle multimodal messages (images, PDFs, files) via raw Anthropic Messages API.
 * Falls back from Agent SDK because query() only accepts string prompts.
 */
async function runMultimodalQuery(contentBlocks, injectedContext, convId, onEvent, abortSignal) {
  ...
  try {
    const stream = await client.messages.stream({
      ...
      max_tokens: 8192,
      system: buildSystemPrompt(convId, injectedContext),
    ...
    if (err.name !== "AbortError") {
      console.error("[claude-multimodal] error:", err.message);
      onEvent("error", { message: err.message });
    }
  }

  return { sessionId: null, resultText };
}
