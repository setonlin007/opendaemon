...
export async function streamClaude({
  ...
  for (const resumeId of attempts) {
    ...
    if (resumeId) {
      ...
      // Session resume counts ALL historical turns (not just this query).
      // A multi-turn conversation can easily exceed the default maxTurns.
      // Set a much higher limit for resume to avoid "max turns" errors.
      opts.maxTurns = Math.max(options.maxTurns * 10, 300);
    }
