...
/**
 * 从 ComfyUI 下载输出图片字节。
 * 带 retry — 当下载走 SSH 反向隧道时，隧道短暂抖动会让 fetch 抛
 * "SocketError: other side closed" (undici 表现为 "terminated")，
 * 几次重试通常能救回。
...
export async function downloadOutput(
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      lastErr = e;
      if (attempt < retries) {
        console.warn(`[comfyui.downloadOutput] attempt ${attempt}/${retries} failed: ${e.message}, retrying in ${retryDelayMs}ms`);
        await new Promise(r => setTimeout(r, retryDelayMs));
      }
    }
  }
  throw new Error(`下载失败（${retries} 次重试后）: ${lastErr?.message || "unknown"}`);
}

/**
 * 连通性探测。
 * 走 SSH 反向隧道时单次握手可能 1-3s 且偶发超时，用 retry 吸收抖动。
 */
export async function ping(comfyUrl = DEFAULT_COMFY_URL, timeoutMs = 8000, retries = 2) {
