...
import { queueEvaluation } from "./evaluator.mjs";
import { loadPrompt, loadSystemPrompt } from "./prompts.mjs";
...
export function buildReflectionPrompt(traces, goals, existingKnowledge) {
  ...
  // Load safety constraints (system-level, cannot be overridden)
  const safetyConstraints = loadSystemPrompt("safety");

  // Load reflection prompt from file (user-configurable, supports A/B override)
  const prompt = loadPrompt("reflection", {
    goals: goals || "(No goals defined yet)",
    trace_count: String(traces.length),
    traces: formattedTraces || "(No traces available)",
    existing_knowledge: existingKnowledge || "(Empty — this is the first reflection)",
    safety_constraints: safetyConstraints,
  });

  // If prompt file is missing, return a minimal fallback
  if (!prompt) {
    return `Analyze these ${traces.length} interaction traces and extract insights.\n\n${formattedTraces}\n\n${safetyConstraints}`;
  }
...
export function parseReflectionOutput(text) {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (category && title && content) {
      const validCategories = ["preferences", "patterns", "domain", "rules", "automation"];
      insights.push({
...
export function acceptPendingInsight(id) {
  ...
  const knowledgeEntry = addKnowledge(insight.category, insight.title, insight.tags, insight.content, "reflection", insight.confidence);
  getDb().prepare("UPDATE pending_insights SET status = 'accepted' WHERE id = ?").run(id);

  // Queue evaluation for the newly accepted knowledge entry
  try {
    queueEvaluation(knowledgeEntry.id);
  } catch (err) {
    console.error(`[reflect] failed to queue evaluation for knowledge ${knowledgeEntry.id}:`, err.message);
  }
...
export function processReflectionResult(llmOutput, { engineId, traceStart, traceEnd, traceCount, triggerReason = "manual", tokens = null, cost = null }) {
  ...
  // Conflict detection: flag insights that might override system safety rules
  const SAFETY_PATTERNS = [
    /不需要.*(确认|验证|审核)/,
    /跳过.*(确认|验证|审核)/,
    /无需.*(确认|验证|审核)/,
    /skip.*(confirm|verif|approv)/i,
    /no.*(confirm|verif|approv).*(needed|required|necessary)/i,
    /直接.*(部署|发布|重启|删除|推送)/,
    /直接.*(deploy|restart|delete|push)/i,
  ];

  for (const insight of insights) {
    const text = `${insight.title} ${insight.content}`;
    const hasConflict = SAFETY_PATTERNS.some(p => p.test(text));
    if (hasConflict) {
      console.warn(`[reflect] conflict detected — insight "${insight.title}" may override safety rules, downgrading to pending`);
      insight._conflictDetected = true;
      // Force to pending review regardless of confidence
      insight.confidence = Math.min(insight.confidence, 0.7);
    }
  }

  const AUTO_ACCEPT_THRESHOLD = 0.9;
  const autoAccept = insights.filter(i => i.confidence >= AUTO_ACCEPT_THRESHOLD && !i._conflictDetected);
  const pending = insights.filter(i => i.confidence < AUTO_ACCEPT_THRESHOLD || i._conflictDetected);
  ...
  // Route automation insights to self-coder (Phase 4)
  const automationInsights = insights.filter(i => i.category === "automation");
  if (automationInsights.length > 0) {
    try {
      // Lazy import to avoid circular deps
      import("./self-coder.mjs").then(({ detectAutomationOpportunity }) => {
        detectAutomationOpportunity([], automationInsights);
      }).catch(() => {}); // Silent fail if self-coder not available
    } catch {}
  }
