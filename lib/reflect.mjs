...
export function buildReflectionPrompt(traces, goals, existingKnowledge) {
  // Check for A/B experiment prompt override
  try {
    const overridePath = join(getProjectRoot(), "data", "reflection_prompt_override.md");
    if (existsSync(overridePath)) {
      const overrideContent = readFileSync(overridePath, "utf-8").trim();
      if (overrideContent) return overrideContent;
    }
  } catch (err) {
    console.error("[reflect] failed to read reflection prompt override:", err.message);
  }
