...
export class MCPManager {
  ...
  /** Clear cached tool list so next listTools() re-fetches from the server. */
  invalidateToolCache() {
    this._tools = null;
  }

  /**
   * Gracefully reload the MCP subprocess.
   * Waits for active requests to complete, then restarts.
   */
  async reload() {
    ...
    try {
      ...
      if (this._pending.size > 0) {
        console.log(`[mcp] waiting for ${this._pending.size} pending request(s)...`);
        const waitStart = Date.now();
        while (this._pending.size > 0 && Date.now() - waitStart < 10000) {
          await new Promise((r) => setTimeout(r, 200));
        }
      }

      // Stop current process
      const wasRunning = !!this._process;
      if (wasRunning) {
        this._stopping = true; // Prevent auto-restart
        if (this._process) {
      ...
      this._restartCount = 0;
      await this.start();
      console.log("[mcp] reload complete");
    } catch (err) {
      console.error("[mcp] reload error:", err);
      throw err;
    }
  }
