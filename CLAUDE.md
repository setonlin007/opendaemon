...
The daemon operates within a multi-project workspace:

```
~/workspace/                         # Root workspace
├── .workspace.json                  # Workspace metadata (project list, config)
├── projects/
│   ├── opendaemon/                  # This project (symlink: ~/opendaemon)
│   └── jarvis-assistant/            # Python AI assistant (symlink: ~/jarvis-assistant)
├── artifacts/                       # Non-code files generated by the daemon
│   ├── documents/                   # Reports, analyses
│   ├── data/                        # Excel, CSV, JSON
│   ├── media/                       # Images, audio, video
│   ├── exports/                     # User exports
│   └── temp/                        # Temporary files (periodic cleanup)
└── scripts/                         # Cross-project utilities
```

**Rules:**
- When the user asks for development work and the target project is unclear, ask: "你指的是哪个项目？"
- Non-code artifacts (Excel, documents, images) → save to `~/workspace/artifacts/` subdirectory
- Artifact download links: `/api/workspace/files/{relative_path}`
- Project-specific `data/` files (uploads, DB) still use `/api/files/{path}`
...
- **Phase 6**: Workspace structure (DONE) — multi-project workspace, artifacts API + browser UI, workspace-aware knowledge injection
- **Phase 7**: TBD
